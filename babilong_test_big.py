...
def run(context_length):
    log_path = f"{task_name}_{N_task_to_try}_testLlamaOllama_{context_length}"
    log = Logger(log_path)
    ...
    for task in tasks:
        ...
        try:
            ...
            compute_and_print_metrics(answer, task, trueP, true_len, pred_len, EM, log)
            log("="* 56 + "\n")
...
def compute_and_print_metrics(answer, task, trueP, true_len, pred_len, EM, log):
