...
class LLaMAContrieverGraph(ContrieverGraph):
    def __init__(self, model, system_prompt, api_key, pipeline, device = "cuda"):
...
class InstructionGraph(LLaMAContrieverGraph):
    def __init__(self, model, system_prompt, api_key, pipeline, device="cuda"):
        super().__init__(model, system_prompt, api_key, pipeline, device)

    def update(self, observation, prev_subgraph, log):        
        example = [re.sub(r"Step \d+: ", "", triplet) for triplet in prev_subgraph][:3]
        prompt = prompt_extraction_current.format(observation = observation, example = example)
        response, _ = self.generate(prompt, t = 0.001)
        new_triplets_raw = process_triplets(response)
...
class GoldenGraph(LLaMAContrieverGraph):
    def __init__(self, model, system_prompt, api_key, pipeline, device="cpu"):
        super().__init__(model, system_prompt, api_key, pipeline, device)

    def add_dialog(self, triplets, dialog):
        self.add_triplets(triplets)
        obs_embedding = self.retriever.embed(dialog)
        obs_value = [self.convert(triplets), obs_embedding]
        self.obs_episodic[dialog] = obs_value
